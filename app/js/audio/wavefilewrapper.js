...
export class WaveFileWrapper {
  ...
  parseFile(arrayBuffer) {
    ...
    while (true) {
      const identifier = chunkView.getInt32(
        OTHER_CHUNK_OFFSET_IDENTIFIER + offset
      );

      // we found the data chunk
      if (identifier === DATA_IDENTIFIER) break;
  ...
  parseAndVerifyFormatChunk(dataView) {
    ...
    if (audioFormat === 1) this.audioFormat = PCM_FORMAT;
    else if (audioFormat === 3) this.audioFormat = IEEE_FLOAT_FORMAT;
    else
      throw new InvalidFormatChunkError(
        "Only PCM and IEEE Float formats are supported"
      );

    this.nbrOfChannels = dataView.getInt16(FORMAT_OFFSET_NBR_OF_CHANNELS, true);
    this.samplesPerSecond = dataView.getInt32(
      FORMAT_OFFSET_SAMPLES_PER_SECOND,
      true
    );
    ...
    if (
      ...
      throw new InvalidFormatChunkError(
        `Bits per sample are ${bitsPerSample}, but have to be 16, 24 or 32 for PCM format`
      );
    else if (
      ...
      throw new InvalidFormatChunkError(
        `Bits per sample are ${bitsPerSample}, but have to be 32 or 64 for IEEE Float format`
      );
 ...
 */
export class FileDurationTooLongError extends Error {
  /**
   * Constructor for FileDurationTooLongError
   *
   * @param message{string}
   */
  constructor(message) {
    super(message);
    this.name = "FileDurationTooLongError";
  }
}
